...
if(mysqli_num_rows($query_check)<1)
{
	...
	if($available==0)
	{
		$sql=mysqli_query($con,"insert into tblpatient(Docid,PatientName,PatientContno,PatientEmail,PatientGender,cabin_type,cabin_price,c_number,day,PatientAdd,PatientAge,PatientMedhis) values('$docid','$patname','$patcontact','$patemail','$gender','$cabin_type','$cabin_price','$c_number','$day','$pataddress','$patage','$medhis')");
		...
		if($sql)
		{
		    ...
		    window.location.href='patient-admit.php';
		    </script>
		    ";
	...
	else
	{
	    echo"
	    <script>
	    alert ('Cabin number not available');
	    window.location.href='patient-admit.php';
	    </script>
	    ";
...
else
{
	...
	echo"
    <script>
    alert ('Cabin number already patient added');
    window.location.href='patient-admit.php';
    </script>
    ";
}
...
?>
			...
			<div class="form-group">
				...
				<select name="cabin_price" class="form-control" onChange="getdoctor(this.value);" required="required">
					<option value="">Select Cabin price</option>
<?php $ret=mysqli_query($con,"select * from cabin_type");
while($row=mysqli_fetch_array($ret))
{
?>
					<option value="<?php echo htmlentities($row['cabin_price']);?>">
						<?php echo htmlentities($row['cabin_price']);?>
					</option>
					<?php } ?>
